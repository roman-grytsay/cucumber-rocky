...
public class Reflections {
    ...
    public <T> Collection<? extends T> instantiateSubclasses(Class<T> parentType, String packageName, Class[] constructorParams, Object[] constructorArgs) {
        ...
        classFinder.getDescendants(parentType, packageName).stream()
                .filter(clazz -> Utils.isInstantiable(clazz) && hasConstructor(clazz, constructorParams))
                .forEach(clazz -> result.add(newInstance(constructorParams, constructorArgs, clazz)));
        return result;
    }
